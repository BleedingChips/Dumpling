...
namespace Dumpling
{
	...
	bool IGHeadUpDisplay::Draw(PassRenderer& renderer)
	{
		std::lock_guard lg(ImGuiContextMutex);
		return DrawTo(renderer);
	}

	IGHeadUpDisplay::Ptr IGHeadUpDisplay::Create(Form& form, FrameRenderer& renderer, IGWidget::Ptr top_weight, std::pmr::memory_resource* resource)
	{
#ifdef _WIN32
