...
namespace PineApple::CharEncode
{
	...
	struct Result
	{
		...
		Result& Add(std::size_t InputUsed, std::size_t OutputUsed) {
			UsedChar += 1;
	...
	template<> struct Encoding<char, char> {
		...
		static Result RequireOutputSpace(From const* Input, std::size_t InputSpace, std::size_t MaxOutputSpace, std::size_t MaxCharLimited);
	...
	template<> struct Encoding<char, wchar_t> {
		...
		static Result RequireOutputSpace(From const* Input, std::size_t InputSpace, std::size_t MaxOutputSpace, std::size_t MaxCharLimited) { return Encoding<char, WCHAR_IMP_TYPE>::RequireOutputSpace(Input, InputSpace, MaxOutputSpace, MaxCharLimited); }
	...
	template<> struct Encoding<char16_t, wchar_t> {
		...
		static Result RequireOutputSpace(From const* Input, std::size_t InputSpace, std::size_t MaxOutputSpace, std::size_t MaxCharLimited) { return Encoding<char16_t, WCHAR_IMP_TYPE>::RequireOutputSpace(Input, InputSpace, MaxOutputSpace, MaxCharLimited); }
	...
	template<> struct Encoding<wchar_t, char16_t> {
		...
		static Result RequireOutputSpace(From const* Input, std::size_t InputSpace) noexcept { return RequireOutputSpace(Input, InputSpace, std::numeric_limits<std::size_t>::max(), std::numeric_limits<std::size_t>::max()); }
		static Result RequireOutputSpaceMaxOutput(From const* Input, std::size_t InputSpace, std::size_t MaxOutputSpace) { return RequireOutputSpace(Input, InputSpace, MaxOutputSpace, std::numeric_limits<std::size_t>::max()); }
		static Result RequireOutputSpaceMaxRequire(From const* Input, std::size_t InputSpace, std::size_t MaxRequire) { return RequireOutputSpace(Input, InputSpace, std::numeric_limits<std::size_t>::max(), MaxRequire); }
	...
	{
		Wrapper(Type const* Input, size_t Length) : mStorage(Input, Length) {}
		Wrapper(std::basic_string_view<Type> Input) : mStorage(Input) {}
		template<typename ToType> std::basic_string<ToType> To() {
			Result Re = Encoding<Type, ToType>::RequireOutputSpace(mStorage.data(), mStorage.size(), std::numeric_limits<size_t>::max(), std::numeric_limits<size_t>::max());
		...
		template<typename Tar, typename Allocator>
		Wrapper(std::basic_string<Type, Tar, Allocator> const& Input) : mStorage(Input) {}
	private:
