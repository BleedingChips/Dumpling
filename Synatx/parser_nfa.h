...
namespace Potato::Parser::NFA
{
	...
	struct Consumer
	{
		...
		//Processer(Table const& ref, std::u32string_view LastCode) : ref(ref), LastCode(LastCode) {}
		//Processer(const Processer&) = default;
		//Consume& operator=(Processer const&) = default;
		std::optional<Travel> operator()(Table const& Ref, std::u32string_view String);
