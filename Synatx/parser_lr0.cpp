...
{
	std::any Processer::operator()(Table const& Table, History const& history, std::any(*Function)(void*, Step const&, Element*), void* FunctionBody)
	{
		std::vector<Element> DataBuffer;
		for (auto& ite : history.steps)
		{
			if (ite.value.IsTerminal())
			{
				auto Result = (*Function)(FunctionBody, ite, nullptr);
				...
				auto Result = (*Function)(FunctionBody, ite, DataBuffer.data() + CurrentAdress);
	...
	bool HandleInputToken(Table const& Table, std::vector<Processer::Step>& Steps, SearchElement& SE, Symbol Sym, size_t token_index)
	{
		...
		for (size_t i = 0; i < Nodes.shift_count; ++i)
		{
			...
			if (TarShift.RequireSymbol == Sym)
			{
				...
				{
					Processer::Step ShiftStep;
					ShiftStep.value = Sym;
					ShiftStep.terminal.token_index = token_index;
	...
	Processer::History Processer::operator()(Table const& Table, Symbol const* TokenArray, size_t TokenLength)
	{
		std::vector<Step> Steps;
		size_t MaxTokenUsed = 0;
		std::vector<Step> BackupSteps;
		std::vector<std::tuple<SearchElement, size_t>> SearchStack;;
		SearchElement CurSearchIte = { 0, {0}, 0 };
		do
		{	
