...
namespace Potato::Parser::Format
{
	...
	static Parser::NFA::Table table = Parser::NFA::CreateTable(PatternRex, 4);


	PatternRef CreatePatternRef(std::u32string_view Ref)
	{
		auto Result = Parser::NFA::Process(table, Ref);
...
namespace Potato::Parser::Format
{
	...
	static NFA::Table FormatTable = NFA::CreateTableReversal(FormatRex, 2);
	...
	std::u32string Formatter<float>::operator()(std::u32string_view par, float input) { 
		...
		return Encoding::EncodingWrapper<char>(std::string_view{Buffer, t}).To<char32_t>();
	}
